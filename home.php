...
if(!isset($_SESSION['loggedIn'])) {
	...
	$sql = "SELECT deal_id FROM {$p}deal_status WHERE member_id = '{$member_id}' AND member_status is null";
	$stmt = $db->query($sql);
...
<div class="container">
    ...
    <div class="navbar-inner">
      <div class="container">
        ...
        <div class="nav-collapse collapse in" id="nav-collapse-01">
                  <ul class="nav">
                    <li class=""><a href="#fakelink">Initiate Deal</a></li>
  ...
  <li>
    <a href="waitingAuthorization.php">Waiting Authorization</a>
  </li>
</ul>
</div>
<?php
...
?>
 ...
 <div class="row" id="tabledata">
  ...
  <div class="row">Ending on: <?php echo($row['deal_end']);?>   </div>
   ...
   <div class="col-md-2">
   		<form id="<?php echo("responseForm".$row['deal_id']);?>" method="POST" action="initiatedDeals.php">
	   		<span id="<?php echo($member_id);?>"><?php echo($member_name);?></span>
	   		<span id="decide_yes" class="glyphicon glyphicon-ok" onclick="clickedYes(<?php echo($row['deal_id']);?>, 'YES')"></span>&nbsp; 
	   		<span id="decide_no" class="glyphicon glyphicon-remove" onclick="clickedNo(<?php echo($row['deal_id']);?>, 'NO')"></span><br>
	   		<input type="hidden" value="<?php echo($row['deal_id']);?>" name="deal_id" id="deal_id">
	   		<input type="hidden" value="" name="response" id="<?php echo("response".$row['deal_id']);?>">
